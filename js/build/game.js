...
var Game = /** @class */ (function () {
    ...
    Game.prototype.createScene3 = function () {
        var scene = new BABYLON.Scene(this.engine);
        scene.clearColor = new BABYLON.Color4(0, 0, 0, 1);
        var camera = new BABYLON.ArcRotateCamera("ArcRotateCamera", 0, Math.PI / 4, 100, new BABYLON.Vector3(0, 0, 0), scene);
        camera.setTarget(BABYLON.Vector3.Zero());
        camera.attachControl(this.canvas, false);
        var light = new BABYLON.PointLight("PointLight", new BABYLON.Vector3(5, 10, 2), scene);
        ...
        for (var _i = 0, _a = map.regions; _i < _a.length; _i++) {
            var region = _a[_i];
            for (var _b = 0, _c = region.territories; _b < _c.length; _b++) {
                var territory = _c[_b];
                var shape = [];
                for (var _d = 0, _e = territory.borders; _d < _e.length; _d++) {
                    var border = _e[_d];
    ...
    Game.prototype.createUI = function (scene) {
        ...
        btnScene3.onPointerUpObservable.add(function () {
            ...
            var handle = window.setTimeout(function () {
                _this.scene.dispose();
                _this.scene = _this.createScene3();
            }, 200);
        });
